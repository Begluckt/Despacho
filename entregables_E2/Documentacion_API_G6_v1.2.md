...
2. **Paginación Estándar:** Las respuestas que devuelven listas incluyen un objeto `pagination` unificado con `total`, `page`, `pageSize`, `totalPages`, `hasNext` y `hasPrev`.
3. **Manejo Unificado de Errores:** En caso de fallas, el servicio responde con una estructura unificada:
   ```json
   {
     ...
     "details": [
       {
         "field": "body.packages.0.weightKg",
...
- **Entrada:** La ciudad de destino (`city`) y una lista de paquetes (`packages`). Cada paquete define su `originCd` (Enum con valores: NORTE, CENTRO, SUR), peso real y dimensiones (Largo, Ancho, Alto).
...
- **Salida:** `totalShippingCost` como valor numérico entero (Money) en CLP.
